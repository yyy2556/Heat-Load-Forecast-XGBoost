...
原始数据来自 Kaggle 公开数据集 DHS Substation Data。本项目使用的 CSV 文件（`heat_exchange_data.csv` 及 `heat_exchange_data_with_iforest.csv`）由配套项目 [Heat-Exchange-Station-Anomaly-Detection](https://github.com/yyy2556/Heat-Exchange-Station-Anomaly-Detection/tree/main) 生成，读取时仅使用其中三列。你也可以将自己的换热站数据整理为相同格式后使用。
...
序列开头没有足够历史值的滞后特征使用 0 填充。数据按时间排序，并使用前 80% 训练、后 20% 测试，不随机打乱顺序，避免时间泄漏。
...
- 仅使用室外温度、时间特征和滞后负荷，未加入太阳辐射、风速等外部气象变量
- 未使用滚动均值、滚动标准差和温度变化率等统计特征
...
1. 增加 6 小时滚动均值和滚动标准差
2. 使用 `sin/cos` 周期编码表达小时和星期周期
3. 接入风速、太阳辐射等外部气象数据
4. 比较全工况模型与采暖季/非采暖季分工况模型
