...
[![Version](https://img.shields.io/badge/version-v1.2-blue)](https://github.com/yyy2556/Heat-Load-Forecast-XGBoost)
[![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![Core Scenario MAPE](https://img.shields.io/badge/core%20scenario%20MAPE-16.07%25-brightgreen)](#运行结果示例)
...
| --- | ---: | ---: | ---: | ---: |
| 稳定供热，`heat_power >= 30 kW` | 1,152 | 28.1972 kW | 21.34% | 23.79% |
| 采暖季 + 白天 + 稳定供热 | 401 | 18.8833 kW | 16.07% | 16.28% |

全量测试集上的模型对比：

| 模型 | 测试集样本数 | MAE | sMAPE |
| --- | ---: | ---: | ---: |
| XGBoost | 2,280 | 28.8024 kW | 106.74% |
| 线性回归基线 | 2,280 | 35.7650 kW | 111.40% |

线性回归基线与 XGBoost 使用完全相同的训练集和测试集特征。结果表明，在当前特征和数据切分下，XGBoost 的全量测试集 MAE 和 sMAPE 均优于线性回归基线；这说明非线性模型在该任务上具有一定收益，但不代表模型已经具备全工况生产部署能力。

这些切片结果适合用于说明模型在核心业务工况下的表现，但不应被表述为全工况指标或生产性能承诺。

其中，最适合对外展示的业务核心时段是“采暖季 + 白天 + 稳定供热”工况：样本数 401，MAE 为 18.8833 kW，MAPE 为 16.07%，sMAPE 为 16.28%。该切片用于说明模型在稳定供热场景下的实际可用性，不代表模型整体泛化能力提升。

### v1.2 数据清洗结果

v1.2 在建模前只针对三列建模数据进行清洗，不修改原始 CSV 文件：

- 原始行数：11,399
- `heat_power < 0` 替换为 `NaN`：0 行
- `outside_temp` 越界（低于 -40°C 或高于 50°C）：0 行
- 线性插补数量：0
- 前向填充数量：0
- 最终剩余缺失值：0

本次运行表明数据中没有发现需要修复的负热功率、越界室外温度或缺失值；`heat_power == 0` 被保留，用于表示真实停机或低负荷状态。
...
| v1.2 | 2026-08-12 | 增加建模数据清洗与插补日志，并加入线性回归基线对比 |
